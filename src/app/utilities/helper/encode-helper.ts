export class EncodeHelper {
  ...
  public static encodeEmail(email: string) {
  ...
  public static encodeMobile(mobile: string) {
    if (mobile.includes('-')) {
  ...
  public static encodePhone(phone: string) {
  ...
  private static replaceAll(slice: string, symbol = '*') {
