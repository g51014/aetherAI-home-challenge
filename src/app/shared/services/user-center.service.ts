...
import { DatabaseService } from '@utilities/abstract/database-service.abstract';
  ...
  public fetchUser(uid: string): Promise<IUser> {
    return new Promise<IUser>((resolve) => {
      this.fetch()
        ...
        .subscribe(
          (user: IUser) => {
            user ? this.$logger.systemMessage(`user ${user.uid} profile has been successfully fetched.`) :
